...
def show_industry():
    st.markdown('<h1>宏观与产业链</h1>', unsafe_allow_html=True)
    st.caption("全球光伏 · 储能 · 锂电池出货与成本趋势 · 2026E 预测")

    region = st.selectbox("区域", ["全球", "北美", "中国", "欧洲", "亚太", "拉美", "中东非"], key="ind_region",
    ...
    with k1: st.metric("光伏出货量 2026E", f"{pv_2026} GW")
    with k2: st.metric("储能出货量 2026E", f"{ess_2026} GWh")
    with k3: st.metric("锂电池出货量 2026E", f"{lith_2026} GWh")
    with k4: st.metric("系统成本 2026E", f"¥{cost_2026:.2f}/Wh")
    ...
    with col1:
        st.markdown('<h3>光伏 + 储能出货增长</h3>', unsafe_allow_html=True)
        ...
        fig.add_trace(go.Bar(name="光伏 (GW)", x=hist_years, y=pv_hist, marker_color="rgba(201,169,110,0.8)"))
        fig.add_trace(go.Bar(name="储能 (GWh)", x=hist_years, y=ess_hist, marker_color="rgba(91,141,184,0.8)"))

        if fcst_years:
            pv_fcst = [pv_vals[i] for i, y in enumerate(years) if y == "2026E"]
            ess_fcst = [ess_vals[i] for i, y in enumerate(years) if y == "2026E"]
            fig.add_trace(go.Bar(name="光伏 (GW) 2026E", x=fcst_years, y=pv_fcst,
                                 marker_color=f"rgba(201,169,110,{FORECAST_BAR_ALPHA})",
                                 marker_line=dict(width=1.5, color="rgba(201,169,110,0.5)")))
            fig.add_trace(go.Bar(name="储能 (GWh) 2026E", x=fcst_years, y=ess_fcst,
    ...
    with col2:
        st.markdown('<h3>锂电池出货与碳酸锂价格</h3>', unsafe_allow_html=True)
        ...
        fig2.add_trace(go.Bar(name="锂电池 (GWh)", x=hist_years, y=lith_hist, marker_color="rgba(91,141,184,0.8)"))

        if fcst_years:
            lith_fcst = [lith_vals[i] for i, y in enumerate(years) if y == "2026E"]
            fig2.add_trace(go.Bar(name="锂电池 (GWh) 2026E", x=fcst_years, y=lith_fcst,
                                  marker_color=f"rgba(91,141,184,{FORECAST_BAR_ALPHA})",
                                  marker_line=dict(width=1.5, color="rgba(91,141,184,0.5)")))

        price_hist = [price_vals[i] for i, y in enumerate(years) if y != "2026E"]
        fig2.add_trace(go.Scatter(x=hist_years, y=price_hist, name="碳酸锂价格",
        ...
        if fcst_years:
            ...
            fig2.add_trace(go.Scatter(x=fcst_x, y=fcst_y, name="碳酸锂价格 (E)",
    ...
    with col3:
        st.markdown('<h3>系统成本下行</h3>', unsafe_allow_html=True)
        ...
        fig3.add_trace(go.Scatter(x=hist_years, y=cost_hist, name="系统成本",
        ...
        if fcst_years:
            ...
            fig3.add_trace(go.Scatter(x=fcst_x, y=fcst_y, name="系统成本 (E)",
    ...
    with col4:
        st.markdown('<h3>全球储能区域结构 · 2026E</h3>', unsafe_allow_html=True)
    ...
    st.markdown('<h3>行业数据表</h3>', unsafe_allow_html=True)
    all_ind = get_industry_data(region=region)
    if all_ind:
        df = pd.DataFrame(all_ind)
        display = df.rename(columns={"category": "类别", "metric_name": "指标", "metric_value": "数值",
                                     "unit": "单位", "period": "周期", "region": "区域"})
        cols = [c for c in ["类别", "指标", "数值", "单位", "周期", "区域"] if c in display.columns]
