...
def show_data_entry():
    ...
    st.caption("Enter / update shipment, cost, and financial data")
    ...
    with col1: selected_name = st.selectbox("Company", list(comp_names.keys()), key="entry_comp", label_visibility="collapsed")
    with col2: period = st.selectbox("Period", ["2022", "2023", "2024", "2025", "2026E", "2025Q1", "2025Q2", "2025Q3", "2025Q4"], key="entry_period", label_visibility="collapsed")
    with col3: data_type = st.selectbox("Data Type", ["Shipment", "Cost / Pricing", "Financials"], key="entry_type", label_visibility="collapsed")
    ...
    if data_type == "Shipment":
        with st.form("shipment_form"):
            st.markdown(f'<h3>{comp["name"]} · {period} · Shipment Data</h3>', unsafe_allow_html=True)
            ...
            if st.form_submit_button("Save Shipment Data", use_container_width=True, type="primary"):
                ok, msg = save_shipment(cid, period, {"total": total, "domestic": domestic, "export": export, "residential": residential, "utility": utility, "commercial": commercial})
                st.success(msg) if ok else st.error(msg)

    elif data_type == "Cost / Pricing":
        with st.form("cost_form"):
            st.markdown(f'<h3>{comp["name"]} · {period} · Cost & Pricing</h3>', unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
            ...
            if st.form_submit_button("Save Cost Data", use_container_width=True, type="primary"):
                ok, msg = save_cost(cid, period, {"system_cost": sc if sc > 0 else None, "domestic_price": dp if dp > 0 else None, "domestic_margin": dm, "export_price": ep if ep > 0 else None, "export_margin": em})
                st.success(msg) if ok else st.error(msg)

    elif data_type == "Financials":
        with st.form("fin_form"):
            st.markdown(f'<h3>{comp["name"]} · {period} · Financials</h3>', unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
            ...
            if st.form_submit_button("Save Financial Data", use_container_width=True, type="primary"):
                ok, msg = save_financial(cid, period, {"revenue": revenue, "gross_margin": gross_margin, "net_profit": net_profit, "net_margin": net_margin, "rv_q1": rv_q1, "rv_q2": rv_q2, "rv_q3": rv_q3, "rv_q4": rv_q4})
                st.success(msg) if ok else st.error(msg)
